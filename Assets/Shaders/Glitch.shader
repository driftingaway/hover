    ...
    SubShader {
        ...
        Pass {
            ...
            float2 distortion(float2 p, float t) {
                float2 frequency = float2(10, 30.0);
                float2 amplitude = float2(0.02, .55);
                float2 speed = float2(1.0, 1.2);
                float2 offset = float2(-1.1, 0.1);
                float2 distortion = .5 * sin(frequency * p * t + offset);
